...
int crc8_verify_header(const uint8_t *header_buffer) {
    uint8_t computed = crc8_compute(header_buffer, 31); /* 前31字节 */
    return (computed == header_buffer[31]) ? 0 : -1;
}

void crc8_fill_header(uint8_t *header_buffer) {
    header_buffer[31] = crc8_compute(header_buffer, 31);
}
